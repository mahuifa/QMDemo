...
#if USE_WINDOW
PlayImage::PlayImage(QOpenGLWindow::UpdateBehavior updateBehavior, QWindow* parent)
...
void PlayImage::repaint(AVFrame* frame)
{
    ...
    if (!m_texY)   // 初始化纹理
    {
        ...
        // 设置纹理大小(当前使用YUV420格式)                         YUV444    YUV422      YUV420
        m_texY->setSize(frame->width, frame->height);           //   w,h      w,h        w,h
        m_texU->setSize(frame->width / 2, frame->height / 2);   //   w,h      w/2,h      w/2,h/2
        m_texV->setSize(frame->width / 2, frame->height / 2);   //   w,h      w/2,h      w/2,h/2
...
void PlayImage::initializeGL()
{
    ...
    glVertexAttribPointer(texCord, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(GLfloat),
                          reinterpret_cast<const GLvoid*>(
                              3 * sizeof(GLfloat)));   // 指定当前绑定到目标的缓冲区的数据存储中数组中第一个通用顶点属性的第一个组件的偏移量。初始值为0 (一个数组从第几个字节开始读)
    // 启用通用顶点属性数组
